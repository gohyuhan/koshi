...
impl Runtime {
    ...
    /// whose view moves. The tab's name is the explicit `--name` when given
    /// (validated like a rename), otherwise generated by
    /// [`tile_core::naming::generate_name`] avoiding the session's existing
    /// tab names. The root pane runs the default shell in `--cwd`. After the
    ...
    fn handle_new_tab(
        ...
        // Resolve the tab's name before the spawn: the explicit `--name`, or
        // a generated one no existing tab in the session already uses.
        let name = match &args.name {
            Some(name) => name.clone(),
            None => generate_name(NameKind::Tab, |candidate| {
    ...
    fn handle_rename_tab(
        ...
        let events = tab_ops::rename_tab(session, tab_id, args.name.clone());
