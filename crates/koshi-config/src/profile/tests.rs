...
"#;
    ...
    let right = TemplateNode::Split(TemplateSplit {
        ...
        weights: vec![
            SizeWeight {
                primary_constraint: SizeConstraint::Flex(1),
                minimum_cell_count: Some(5),
                preferred_cell_count: Some(20),
                resize_delta: 0,
            },
    ...
    let expected_profile_template = ProfileTemplate {
        tabs: vec![TabTemplate {
...
fn stack_defaults_to_first_member_expanded() {
    ...
    assert_eq!(split.active_child_index, 0);
...
fn min_and_max_percent_size_are_accepted() {
    ...
    assert_eq!(
        minimum_percent_split.weights[0],
    ...
    assert_eq!(
        maximum_percent_split.weights[0],
...
fn min_and_max_cell_size_are_accepted() {
    ...
    assert_eq!(
        maximum_cell_split.weights[0],
        SizeWeight::from_primary_constraint(SizeConstraint::Fixed(65535))
    );
}
...
fn syntax_error_is_the_syntax_variant() {
    let profile_error = parse_profile_text("tab {").unwrap_err();

    let ProfileError::Syntax(diagnostic) = profile_error else {
        panic!("expected a syntax error, got {profile_error:?}");
    };
    assert_eq!(
...
fn invalid_report_names_the_file() {
    let profile_error = parse_profile_text("version 1").unwrap_err();
    assert_eq!(
        profile_error.to_string(),
        "invalid profile file profile/dev.kdl"
    );
}
...
fn missing_version_is_reported() {
    assert_eq!(
        collect_profile_diagnostic_messages("tab {pane}"),
...
fn non_integer_version_is_reported() {
    assert_eq!(
        collect_profile_diagnostic_messages("version \"one\"\ntab {pane}"),
...
fn version_as_property_is_reported() {
    assert_eq!(
        collect_profile_diagnostic_messages("version v=1\ntab {pane}"),
...
fn version_with_two_arguments_is_reported() {
    assert_eq!(
        collect_profile_diagnostic_messages("version 1 2\ntab {pane}"),
        ["`version` takes exactly one integer argument"]
    );
}
...
fn version_above_the_u32_ceiling_is_reported() {
    ...
    assert_eq!(
        collect_profile_diagnostic_messages("version 4294967296\ntab {pane}"),
        ["`version` must be an integer from 1 to 4294967295"]
    );
}
...
fn each_sizing_violation_points_its_caret_at_its_own_node() {
    ...
    assert_eq!(
        collect_profile_diagnostic_spans(
            "version 1\ntab { stack { pane { size 30; min 5 }; pane } }"
        ),
        ["size 30", "min 5 "]
    );
...
#[test]
fn working_directory_arity_is_reported() {
    assert_eq!(
        collect_profile_diagnostic_messages("version 1\ntab { pane { cwd \"a\" \"b\" } }"),
...
#[test]
fn duplicate_working_directory_is_reported() {
    assert_eq!(
...
#[test]
fn non_string_working_directory_is_reported() {
    assert_eq!(
        collect_profile_diagnostic_messages("version 1\ntab { pane {cwd 42} }"),
...
#[test]
fn empty_working_directory_is_reported() {
    assert_eq!(
        collect_profile_diagnostic_messages("version 1\ntab { pane { cwd \"\" } }"),
        ["`cwd` takes one non-empty string"]
    );
...
#[test]
fn working_directory_with_children_is_reported() {
    assert_eq!(
        collect_profile_diagnostic_messages("version 1\ntab { pane { cwd \"a\" {} } }"),
...
#[test]
fn working_directory_with_a_nul_is_reported() {
    assert_eq!(
