...
fn collect_keybinding_diagnostic_messages(keybinding_text: &str) -> Vec<String> {
    match parse_keybinding_text(keybinding_text) {
        Err(KeybindingParseError::Invalid { diagnostics, .. }) => diagnostics
            ...
            .collect(),
        Err(KeybindingParseError::Syntax(syntax_error)) => {
            panic!("expected schema errors, got syntax: {syntax_error}")
        }
...
fn invalid_kdl_syntax_is_a_syntax_error() {
    let parse_error = parse_keybinding_text("mode \"normal\" {").expect_err("unclosed block");
    assert_eq!(
        parse_error.to_string(),
        "config parse error in keybinding.kdl"
    );
    assert!(
        matches!(parse_error, KeybindingParseError::Syntax(_)),
        "got: {parse_error:?}"
    );
}
...
"#,
    ...
    assert_eq!(diagnostic_messages, ["duplicate `remove \"<Tab>\"`"]);
}
...
fn duplicate_setting_node_is_rejected() {
    let diagnostic_messages =
        collect_keybinding_diagnostic_messages("chord-timeout-ms 500\nchord-timeout-ms 600");
    assert_eq!(
        diagnostic_messages,
        ["`chord-timeout-ms` is declared more than once"]
    );
}

#[test]
fn unknown_top_level_node_is_rejected() {
    let diagnostic_messages = collect_keybinding_diagnostic_messages("keybindings { }");
    assert_eq!(
        diagnostic_messages,
        ["unknown key `keybindings`; did you mean `version`?"]
    );
}
...
fn unknown_node_inside_mode_is_rejected() {
    let diagnostic_messages =
        collect_keybinding_diagnostic_messages(r#"mode "normal" { unbind "<Tab>" }"#);
    assert_eq!(
        diagnostic_messages,
...
fn bind_arity_violations_are_rejected() {
    let expected_diagnostic_message =
        "`bind` takes exactly two string arguments: a key sequence and an action reference";
    assert_eq!(
        collect_keybinding_diagnostic_messages(r#"mode "normal" { bind "<C-t>" }"#),
        [expected_diagnostic_message]
    );
    assert_eq!(
        collect_keybinding_diagnostic_messages(
            r#"mode "normal" { bind "<C-t>" "core:new-tab" "extra" }"#
        ),
        [expected_diagnostic_message]
    );
    assert_eq!(
        collect_keybinding_diagnostic_messages(
            r#"mode "normal" { bind key="<C-t>" action="core:new-tab" }"#
        ),
        [expected_diagnostic_message]
    );
    assert_eq!(
        collect_keybinding_diagnostic_messages(
            r#"mode "normal" { bind "<C-t>" "core:new-tab" { } }"#
        ),
        ["`bind` takes no children"]
    );
    assert_eq!(
        collect_keybinding_diagnostic_messages(r#"mode "normal" { bind 1 2 }"#),
...
fn remove_arity_violations_are_rejected() {
    assert_eq!(
        collect_keybinding_diagnostic_messages(r#"mode "normal" { remove }"#),
        ["`remove` takes exactly one string argument"]
    );
    assert_eq!(
        collect_keybinding_diagnostic_messages(r#"mode "normal" { remove "<Tab>" "extra" }"#),
        ["`remove` takes exactly one string argument"]
    );
    assert_eq!(
        collect_keybinding_diagnostic_messages(r#"mode "normal" { remove "<Tab>" { } }"#),
        ["`remove` takes no children"]
    );
    assert_eq!(
        collect_keybinding_diagnostic_messages(r#"mode "normal" { remove 1 }"#),
...
fn mode_name_must_be_a_string() {
    assert_eq!(
        collect_keybinding_diagnostic_messages(r#"mode 1 { }"#),
...
fn bind_of_an_empty_key_is_rejected() {
    assert_eq!(
        collect_keybinding_diagnostic_messages(r#"mode "normal" { bind "" "core:new-tab" }"#),
...
fn duplicate_leader_node_is_rejected() {
    assert_eq!(
        collect_keybinding_diagnostic_messages("leader \"<C-p>\"\nleader \"<A-p>\""),
...
fn bad_leader_value_is_rejected() {
    assert_eq!(
        collect_keybinding_diagnostic_messages(r#"leader "<C-""#),
        ["invalid key `<C-`: missing closing `>`"]
    );
}
...
fn bad_unlock_alternative_value_is_rejected() {
    assert_eq!(
        collect_keybinding_diagnostic_messages(r#"unlock-alternative "not a chord""#),
...
fn newer_version_is_rejected() {
    assert_eq!(
        collect_keybinding_diagnostic_messages("version 999"),
        [format!(
...
fn version_zero_is_rejected() {
    assert_eq!(
        collect_keybinding_diagnostic_messages("version 0"),
...
fn a_duplicate_version_node_is_reported_once_and_the_second_is_not_checked() {
    ...
    assert_eq!(
        collect_keybinding_diagnostic_messages("version 1\nversion 999"),
...
fn out_of_range_integer_is_rejected() {
    assert_eq!(
        collect_keybinding_diagnostic_messages("max-chord-depth 300"),
        ["`max-chord-depth` must be an integer from 0 to 255"]
    );
    assert_eq!(
        collect_keybinding_diagnostic_messages("chord-timeout-ms -1"),
        ["`chord-timeout-ms` must be an integer from 0 to 4294967295"]
    );
}
...
fn setting_arity_violations_are_rejected() {
    assert_eq!(
        collect_keybinding_diagnostic_messages("chord-timeout-ms"),
        ["`chord-timeout-ms` takes exactly one integer argument"]
    );
    assert_eq!(
        collect_keybinding_diagnostic_messages("chord-timeout-ms 1 2"),
        ["`chord-timeout-ms` takes exactly one integer argument"]
    );
    assert_eq!(
        collect_keybinding_diagnostic_messages("leader"),
        ["`leader` takes exactly one string argument"]
    );
    assert_eq!(
        collect_keybinding_diagnostic_messages(r#"leader "<C-p>" { }"#),
        ["`leader` takes no children"]
    );
    assert_eq!(
        collect_keybinding_diagnostic_messages(r#"unlock-alternative "<A-u>" { }"#),
    ...
    assert_eq!(
        collect_keybinding_diagnostic_messages("mode { }"),
