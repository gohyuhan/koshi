...
fn push_keeps_every_known_flag_bit() {
    ...
    keyboard_stack.push_keyboard_flags(17);

    assert_eq!(keyboard_stack.get_current_keyboard_flags(), 17);
}
...
fn pop_removes_exactly_the_requested_entries() {
    let mut keyboard_stack = KeyboardStack::default();
    keyboard_stack.push_keyboard_flags(1);
    keyboard_stack.push_keyboard_flags(2);
    keyboard_stack.push_keyboard_flags(4);
...
fn set_then_push_then_pop_restores_the_flags_the_push_covered() {
    ...
    keyboard_stack.set_current_keyboard_flags(1, 1);
...
fn set_mode_one_replaces_the_last_entry() {
    let mut keyboard_stack = KeyboardStack::default();
    keyboard_stack.push_keyboard_flags(1);
    keyboard_stack.push_keyboard_flags(9);
...
fn set_mode_three_clears_from_the_last_entry() {
    let mut keyboard_stack = KeyboardStack::default();
    keyboard_stack.push_keyboard_flags(1);
    keyboard_stack.push_keyboard_flags(13);
...
fn stack_round_trips_through_its_entry_list() {
    let mut keyboard_stack = KeyboardStack::default();
    keyboard_stack.push_keyboard_flags(1);
    keyboard_stack.push_keyboard_flags(24);
