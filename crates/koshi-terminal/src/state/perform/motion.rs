...
impl TerminalState {
    ...
    pub(in crate::state) fn delete_lines_into_scrollback(
        ...
        let should_feed_scrollback = self.active_screen == Screen::Primary && first_row_index == 0;
    ...
    pub(super) fn restore_cursor(&mut self) {
        if let Some(saved) = self.active_cursor().saved {
            ...
            cursor.row = saved.row.min(grid_row_count.saturating_sub(1));
            cursor.column = saved.column.min(grid_column_count.saturating_sub(1));
    ...
    pub(super) fn move_cursor_to(&mut self, target_row_index: u16, target_column_index: u16) {
        let (grid_row_count, grid_column_count) = self.get_active_grid().get_grid_dimensions();
        let cursor = self.active_cursor_mut();
        cursor.row = target_row_index.min(grid_row_count.saturating_sub(1));
        cursor.column = target_column_index.min(grid_column_count.saturating_sub(1));
        cursor.pending_wrap = false;
    }

    /// Park the cursor on `last_column_index`. With autowrap (DECAWM `?7`) on, arm the
    /// deferred-wrap latch: the next glyph wraps before printing. With autowrap
    /// off, clear the latch: the next glyph overwrites `last_column_index` in place,
    ...
    pub(super) fn arm_wrap_latch(&mut self, last_column_index: u16) {
        let should_arm_wrap_latch = self.modes.autowrap;
        let cursor = self.active_cursor_mut();
        cursor.column = last_column_index;
