...
impl Runtime {
    ...
    /// client's outer terminal as OSC 52, which sets the OS clipboard. Trailing
    /// blanks are trimmed or preserved from the current copy config. The
    /// internal `copy_on_select` switch can hold the highlight without copying
    /// once another copy path exists; users cannot set it yet.
