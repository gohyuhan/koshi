...
// ---------------------------------------------------------------------------
// Keybinding edits: set / remove / reset through dispatch
// ---------------------------------------------------------------------------

/// Parse a test key sequence with the default leader and depth.
fn seq(s: &str) -> koshi_core::key::KeySequence {
    parse_sequence(s, koshi_config::key::Leader::default(), 8).expect("test sequence parses")
}

/// The action currently firing on `s` in `mode`, read from the live catalog.
fn bound_action(rt: &Runtime, mode: LockMode, s: &str) -> Option<koshi_core::action::ActionRef> {
    rt.keymap_hints
        .match_sequence(mode, &seq(s))
        .exact
        .map(|binding| binding.action)
}

/// A `core:` action reference.
fn core_ref(name: &str) -> koshi_core::action::ActionRef {
    koshi_core::action::ActionRef::core(name).expect("valid core action name")
}

#[test]
fn set_key_binding_applies_live() {
    let (mut rt, _tx) = new_runtime();
    ...
    let result = rt.dispatch(envelope(Command::SetKeyBinding(SetKeyBindingArgs {
        ...
        action: core_ref("new-tab"),
    })));

    assert!(matches!(result, CommandResult::Ok { .. }));
    assert_eq!(
        bound_action(&rt, LockMode::Normal, "<C-y>"),
        Some(core_ref("new-tab"))
    );
}

#[test]
fn set_key_binding_steals_a_defaulted_key() {
    let (mut rt, _tx) = new_runtime();
    assert_eq!(
        bound_action(&rt, LockMode::Normal, "<A-t>"),
        Some(core_ref("new-tab"))
    );

    let result = rt.dispatch(envelope(Command::SetKeyBinding(SetKeyBindingArgs {
        mode: Some("normal".to_string()),
        sequence: "<A-t>".to_string(),
        action: core_ref("close-pane"),
    })));

    assert!(matches!(result, CommandResult::Ok { .. }));
    assert_eq!(
        bound_action(&rt, LockMode::Normal, "<A-t>"),
        Some(core_ref("close-pane"))
    );
}

#[test]
fn set_key_binding_rejects_an_unregistered_action() {
    let (mut rt, _tx) = new_runtime();

    let result = rt.dispatch(envelope(Command::SetKeyBinding(SetKeyBindingArgs {
        mode: None,
        sequence: "<C-y>".to_string(),
        action: core_ref("no-such-action"),
    })));
    ...
    match result {
        CommandResult::Rejected { reason, help, .. } => {
            ...
            assert_eq!(
                help,
                Some("no action named `core:no-such-action` is registered".to_string())
            );
        }
...
#[test]
fn set_key_binding_rejects_a_coming_soon_action() {
    let (mut rt, _tx) = new_runtime();

    let result = rt.dispatch(envelope(Command::SetKeyBinding(SetKeyBindingArgs {
        mode: None,
        sequence: "<C-y>".to_string(),
        action: core_ref("copy-mode-enter"),
    })));
    ...
    match result {
        CommandResult::Rejected { reason, help, .. } => {
            ...
            assert_eq!(
                help,
                Some("action `core:copy-mode-enter` is not implemented yet".to_string())
            );
        }
        other => panic!("expected rejection, got {other:?}"),
    }
...
#[test]
fn set_key_binding_rejects_an_unknown_mode() {
    let (mut rt, _tx) = new_runtime();
    ...
    match result {
        CommandResult::Rejected { reason, help, .. } => {
            ...
            assert_eq!(
                help,
                Some(
                    "no mode named `visual`; built-in modes are `normal` and `locked`".to_string()
                )
            );
        }
        other => panic!("expected rejection, got {other:?}"),
...
#[test]
fn set_key_binding_cannot_shadow_the_reserved_unlock() {
    let (mut rt, _tx) = new_runtime();

    let result = rt.dispatch(envelope(Command::SetKeyBinding(SetKeyBindingArgs {
        mode: Some("locked".to_string()),
        sequence: "<C-l>".to_string(),
        action: core_ref("new-tab"),
    })));
    ...
    // The guaranteed escape is untouched.
    assert_eq!(
        bound_action(&rt, LockMode::Locked, "<C-l>"),
        Some(core_ref("unlock"))
    );
}

#[test]
fn remove_key_binding_applies_live() {
    let (mut rt, _tx) = new_runtime();
    assert_eq!(
        bound_action(&rt, LockMode::Normal, "<A-t>"),
        Some(core_ref("new-tab"))
    ...
    let result = rt.dispatch(envelope(Command::RemoveKeyBinding(RemoveKeyBindingArgs {
        mode: None,
        sequence: "<A-t>".to_string(),
    })));

    assert!(matches!(result, CommandResult::Ok { .. }));
    assert_eq!(bound_action(&rt, LockMode::Normal, "<A-t>"), None);
}

#[test]
fn remove_key_binding_rejects_an_unbound_key() {
    let (mut rt, _tx) = new_runtime();
    ...
    match result {
        CommandResult::Rejected { reason, help, .. } => {
            ...
            assert_eq!(
                help,
                Some("nothing is bound on `<C-y>` in `normal` mode".to_string())
            );
        }
        other => panic!("expected rejection, got {other:?}"),
    }
...
#[test]
fn remove_key_binding_cannot_remove_the_locked_unlock() {
    let (mut rt, _tx) = new_runtime();
...
#[test]
fn remove_then_set_rebinds_the_key() {
    let (mut rt, _tx) = new_runtime();

    let removed = rt.dispatch(envelope(Command::RemoveKeyBinding(RemoveKeyBindingArgs {
        mode: None,
        sequence: "<A-t>".to_string(),
    })));
    assert!(matches!(removed, CommandResult::Ok { .. }));
    assert_eq!(bound_action(&rt, LockMode::Normal, "<A-t>"), None);

    let set = rt.dispatch(envelope(Command::SetKeyBinding(SetKeyBindingArgs {
        mode: None,
        sequence: "<A-t>".to_string(),
        action: core_ref("close-pane"),
    })));
    assert!(matches!(set, CommandResult::Ok { .. }));
    assert_eq!(
        bound_action(&rt, LockMode::Normal, "<A-t>"),
        Some(core_ref("close-pane"))
    );
...
#[test]
fn reset_key_bindings_for_one_mode_restores_its_defaults() {
    let (mut rt, _tx) = new_runtime();
    ...
    let reset = rt.dispatch(envelope(Command::ResetKeyBindings(ResetKeyBindingsArgs {
        mode: Some("normal".to_string()),
    })));
    assert!(matches!(reset, CommandResult::Ok { .. }));
...
#[test]
fn reset_key_bindings_without_a_mode_restores_the_whole_section() {
    let (mut rt, _tx) = new_runtime();

    // A reloaded keybinding file customizes a timing field; a manual edit
    // customizes the keymap.
    let outcome = rt.reload_keybindings(PartialKeybindingsConfig {
        chord_timeout_ms: Some(900),
        ..PartialKeybindingsConfig::default()
    });
    assert!(outcome
        .events
        .iter()
        .all(|event| matches!(event, koshi_core::event::Event::ConfigReloaded(_))));
    assert_eq!(rt.config.keybindings.chord_timeout_ms, 900);
    let set = rt.dispatch(envelope(Command::SetKeyBinding(SetKeyBindingArgs {
        mode: None,
        sequence: "<C-y>".to_string(),
        action: core_ref("new-tab"),
    ...
    let defaults = koshi_config::types::KeybindingsConfig::default();
    assert_eq!(
        rt.config.keybindings.chord_timeout_ms,
...
#[test]
fn manual_bindings_survive_a_keybinding_file_reload() {
    let (mut rt, _tx) = new_runtime();
    ...
    let outcome = rt.reload_keybindings(PartialKeybindingsConfig::default());
    assert!(outcome
        .events
        .iter()
        .all(|event| matches!(event, koshi_core::event::Event::ConfigReloaded(_))));
...
#[test]
fn keymap_edit_clears_pending_key_sequences() {
    let (mut rt, _tx, client_id, sid) = lock_fixture();
    rt.sessions
        .get_mut(&sid)
        .expect("session")
        ...
        .get_mut(client_id)
        .expect("client")
        .update_pending_key_sequence(Some(koshi_session::client::PendingKeySequence {
            sequence: seq("<C-p>"),
            raw_bytes: vec![vec![0x10]],
            deadline: None,
        }));
