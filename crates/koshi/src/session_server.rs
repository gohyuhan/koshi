...
use koshi_ipc::endpoint::resume_path;
...
///
/// The router leaves a session whose resume file is younger than this alone, so
/// a `koshi list-sessions` running during a swap does not take that session's
/// advertisement off the disk. A resume file older than this means the swap
/// died, and the session is removed as usual.
...
fn bind_socket(
    ...
    let other_users = crate::config::other_users_policy(
        crate::config::load_app_layer().as_ref(),
...
fn resume_readers(
    ...
    rebuilt.load_startup_config(crate::config::load_app_layer());
    rebuilt
}
