...
//! that fails resumes serving, still ignoring the SIGPIPE signal. On Windows
//! it starts the new binary, which waits for the router lock, and then runs
//! its own shutdown and exits.
//!
...
/// Answer a restart request by reading the metadata of the binary at `exe`. A
/// binary that cannot be read is refused. Nothing is torn down either way.
