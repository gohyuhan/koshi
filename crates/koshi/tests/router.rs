...
/// handling changed.
///
/// The binary is readable and not executable, so the check before the exec
/// passes and `execvp` then fails with `EACCES`.
#[cfg(unix)]
#[test]
fn a_failed_restart_leaves_the_router_serving_hung_up_clients() {
    ...
    std::fs::set_permissions(&exe, std::fs::Permissions::from_mode(0o644))
        .expect("the binary is readable and does not run");
