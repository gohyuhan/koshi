...
fn an_otherwise_expression_is_kept_whole() {
    assert_eq!(
        parse_beta_feature_arguments("otherwise = 0"),
        Ok("0".to_string())
    );
    assert_eq!(
        parse_beta_feature_arguments("otherwise = Ok(())"),
        Ok("Ok (())".to_string())
    );
}
...
fn commas_inside_the_expression_do_not_end_it() {
    assert_eq!(
        parse_beta_feature_arguments("otherwise = Err(E::new(1, 2))"),
        Ok("Err (E :: new (1 , 2))".to_string())
    );
    assert_eq!(
        parse_beta_feature_arguments("otherwise = (1, 2)"),
        Ok("(1 , 2)".to_string())
    );
}

/// The literal `()` is an ordinary expression to the parser. `is_unit_expression`
/// decides what the blocked call does with it.
#[test]
fn the_literal_unit_parses_as_an_expression() {
    assert_eq!(
        parse_beta_feature_arguments("otherwise = ()"),
        Ok("()".to_string())
    );
    assert_eq!(
        parse_beta_feature_arguments("otherwise = ( )"),
...
fn a_block_or_control_flow_value_is_kept_whole() {
    ...
    assert_eq!(
        parse_beta_feature_arguments("otherwise = if x { 1 } else { 2 }"),
...
fn an_argument_that_is_not_otherwise_is_rejected() {
    assert_eq!(
        parse_beta_feature_arguments("other = 1"),
...
fn the_name_must_match_otherwise_exactly() {
    ...
    assert_eq!(
        parse_beta_feature_arguments("r#otherwise = 1"),
        Err("expected `otherwise = <expression>`".to_string())
    );
}
...
fn a_keyword_in_place_of_the_name_is_rejected() {
    assert_eq!(
        parse_beta_feature_arguments("fn = 1"),
...
fn a_second_argument_is_rejected() {
    assert_eq!(
        parse_beta_feature_arguments("otherwise = 1, extra = 2"),
