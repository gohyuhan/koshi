...
fn an_empty_tab_policy_survives_a_serde_round_trip() {
    let policy = EmptyTabPolicy::CloseTab;
    let json = serde_json::to_string(&policy).expect("serialize");
...
#[test]
fn the_empty_tab_policy_serializes_as_its_variant_name() {
    assert_eq!(
...
fn an_unknown_empty_tab_policy_fails_to_deserialize() {
    ...
    assert_eq!(
        error.to_string(),
        "unknown variant `KeepOpen`, expected `CloseTab` at line 1 column 10"
    );
}

#[test]
fn a_removed_empty_tab_policy_fails_to_deserialize() {
    let error =
        serde_json::from_str::<EmptyTabPolicy>(r#""RespawnShell""#).expect_err("unknown variant");

    assert_eq!(
        error.to_string(),
        "unknown variant `RespawnShell`, expected `CloseTab` at line 1 column 14"
    );
}
