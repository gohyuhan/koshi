...
//!
//! [`EmptyTabPolicy`] holds one value, its default
//! [`CloseTab`](EmptyTabPolicy::CloseTab): removing a tab's last pane closes
//! the tab, and closing the last tab ends the session. [`LastTabPolicy`] holds
//! one value, its default [`Quit`](LastTabPolicy::Quit).
