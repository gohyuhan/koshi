...
/// in the spawn specification's environment variables are applied last and
...
/// case-insensitively.
pub fn build_environment_overlay(spawn_spec: &SpawnSpec) -> BTreeMap<String, String> {
    let mut environment_overlay = BTreeMap::new();

    // Set the terminal identity for every shell.
    environment_overlay.insert("TERM".to_string(), "xterm-256color".to_string());
    environment_overlay.insert("COLORTERM".to_string(), "truecolor".to_string());
    ...
    match spawn_spec.shell_kind {
        ShellKind::Zsh => {
            environment_overlay.insert("PROMPT_EOL_MARK".to_string(), String::new());
        }
        ShellKind::Bash
        | ShellKind::Fish
    ...
    environment_overlay.extend(spawn_spec.environment_variables.clone());
    environment_overlay
}
