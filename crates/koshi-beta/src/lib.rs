...
//! gate reads the value stored by [`set_beta_features_allowed`] at the start of the body: at
...
///
/// The event has a `function` field containing the function name. Its message
/// says that the function did nothing and names the `koshi.kdl` line that
/// enables it. The message contains the function name unchanged.
///
...
/// as `session::attach`.
///
/// `log_blocked_feature_warning("attach")` emits this message:
/// ``` text
/// `attach` is a beta feature and did nothing; add a top-level `allow-beta-features #true` line to koshi.kdl to run it
/// ```
pub fn log_blocked_feature_warning(function_name: &str) {
    tracing::warn!(
        function = function_name,
        "`{function_name}` is a beta feature and did nothing; add a top-level \
